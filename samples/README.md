...
|---|---|
| [tour](tour.goose) | The core language in one program: values and widths, every array kind, slices, structs, control flow as expressions, several results, optionals, references. |
| [memory](memory.goose) | The memory model, made visible: references that survive growth, scope exit as the only free, cleared scratch buffers, flat nested containers, `copy`, grow-shrink stacks, limited arrays, `reusable` pools. |
...
| [bignum](bignum.goose) | Arbitrary precision on `u32` limbs with `u64` carries: 100!, fib(500), 2^1000, decimal printing, a `format` overload. |
...
| Sample | What it shows |
|---|---|
| [linked_list](linked_list.goose) | A doubly-linked list in a `reusable` pool with `in pool` links and a `self` sentinel: O(1) insert/remove through references, slot reuse, move-to-front. |
...
| Sample | What it shows |
|---|---|
| [calc](calc.goose) | An interactive calculator: tokens as fixed-mode enum values, recursive descent straight to values, variables in a dictionary, every error one `return ... from`. Run with `calc < data/calc.stdin`. |
| [json](json.goose) | A JSON parser and printer: variable-mode nodes in one pool with 4-byte relative links, deep errors with positions, pretty and compact rendering, key and index lookups. |
