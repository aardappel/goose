...
implementation, Goose wins seven of the sixteen under both backends, is level on two, splits
...
allocation anywhere, and beats the Rust DTO by 2.3x while matching
...
**The three consistent losses have different causes.**
...
`calc` (0.82-0.86x) has costs specific to the Goose implementation: a global parse
...
that never leaves L1. Four other comparisons depend on the backend: `lru` is 0.68x under v145 and 1.16x under clang because MSVC will
...
all. Under clang the checks have no measurable cost: it vectorises around them,
as LLVM does for Rust's checked implementation, and the same Goose source runs at 271
...
and with C++ under `__restrict`. Since these tables were measured, the compiler has gained the ability to
propagate the image length from `main` into the kernel. The flat version now
eliminates its checks without the `assert` and runs in 77 ms.

**Peak memory use is 1.9x smaller than idiomatic C++, 1.3x
...
**Eight of the sixteen benchmarks use the standard library without a
measurable slowdown.**
...
hand-written table used for comparison -- a table-sizing difference as much as a
