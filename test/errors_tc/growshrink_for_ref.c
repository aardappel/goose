/* Generated by the Goose compiler from C:\W\goose\test\errors_tc\growshrink_for_ref.goose. Do not edit. */
...
typedef struct { gs_rhdr *hdr; gs_stack *stk; gs_rhdr *fl; gs_stack *flstk; } gs_pref;
...
/* ---- data ---- */
static const char gs_file0[] = "C:/W/goose/test/errors_tc/growshrink_for_ref.goose";
...
/* ---- prototypes ---- */
static void gs_main(int64_t gs_sp);

/* ---- code ---- */
static void gs_main(int64_t gs_sp) {
    gs_rhdr gs;
    GS_ENSURE(gs_sp + 1);
    uint8_t *t5 = GS(gs_sp + 0)->top;
    gs = (gs_rhdr){ t5, 0 };
    gs.len = 3;
    *(int64_t *)t5 = 1;
    t5 += 8;
    ...
    *(int64_t *)t5 = 3;
    t5 += 8;
    for (int64_t t0 = 0; t0 < ((gs.len)); t0++) {
        int64_t * x = &(*(int64_t *)(((gs.base)) + t0 * 8));
        if ((uint8_t)((int64_t)((*x)) == (int64_t)(2))) {
            int64_t t3 = (gs.len) - 1;
            if (t3 < 0) gs_abort(GS_E_POP, gs_file0, 5);
            gs.len = (int64_t)t3;
            t5 -= 8;
            int64_t t4 = *(int64_t *)t5;
        }
    }
    t5 = gs.base;
    GS(gs_sp + 0)->top = t5;
...
static void gs_init_globals(void) {
}
